...
void initHw()
{
    // Configure HW to work with 16 MHz XTAL, PLL enabled, sysdivider of 5, creating system clock of 25 MHz
...
{
    __asm("WMS_LOOP0:   MOV  R1, #4");          // 1
    __asm("WMS_LOOP1:   SUB  R1, #1");          // 4
    __asm("             CBZ  R1, WMS_DONE1");   // 3+1*3
    __asm("             NOP");                  // 3
    __asm("             B    WMS_LOOP1");       // 3*2 (speculative, so P=1)
    __asm("WMS_DONE1:   SUB  R0, #1");          // 1
    __asm("             CBZ  R0, WMS_DONE0");   // 1 + 3
